...
/**
 * VPS Raw Data Logger (Secure Version)
 * 
 ...
 * 
 * It handles self-signed certificates and provides detailed logging of all data.
 */
...
const ALLOW_INSECURE = process.env.ALLOW_INSECURE === 'true' || true; // Default to allowing insecure connections
...
console.log(`- Allow insecure connections: ${ALLOW_INSECURE ? 'Yes' : 'No'}`);
console.log('=======================================\n');
...
class RawDataLogger extends EventEmitter {
  ...
  async connect() {
    ...
    try {
      ...
      // Configure WebSocket with options to handle self-signed certificates
      const wsOptions = {
        rejectUnauthorized: !ALLOW_INSECURE,
        agent: ALLOW_INSECURE && connectionUrl.startsWith('wss://') ? 
          new https.Agent({ rejectUnauthorized: false }) : undefined
      };
      
      this.log(`WebSocket options: ${JSON.stringify(wsOptions)}`);
      this.socket = new WebSocket(connectionUrl, wsOptions);
      ...
      this.socket.on('error', (error) => {
        ...
        if (error.message.includes('401')) {
          ...
          this.log('To allow insecure connections, set ALLOW_INSECURE=true in your environment');
          this.log('Or run this script with: ALLOW_INSECURE=true node scripts/vps-raw-data-logger-secure.js');
        }
      });
