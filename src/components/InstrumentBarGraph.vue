...
const axisWidth = 10; // Width for the axis, matching line graph
...
function redraw() {
  ...
  const safeData = Array.isArray(props.data) && props.data.length > 0 
    ? props.data 
    : [0, 0, 0, 0, 0];
  ...
  const y = d3.scaleLinear()
    ...
    .range([h, 0]); // Invert the range for proper bar height calculation
  ...
  mainGroup.selectAll("rect")
    ...
    .attr("x", (d, i) => x(i))
    .attr("y", d => y(d))
    .attr("width", w / props.maxHistoryLength - 1)
    .attr("height", d => h - y(d))
