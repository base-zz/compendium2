...
async function bridgeStateToRelay() {
  try {
    ...
    setInterval(() => {
      // console.log('[SERVER] Bridging stateData to relay:', JSON.stringify(stateData, null, 2));
      stateManager.applyDomainUpdate(stateData);
    }, 1000);
    console.log('[SERVER] State bridge: canonical stateData -> relay stateManager is active.');
  } catch (err) {
